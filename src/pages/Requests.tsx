  ...
  useEffect(() => {
    const getUsers = async () => {
      try {
        ...
        const requestedUsers = await getRequest(
          "/admins/petitions",
          accessToken,
        );
  ...
  const getPaginationItems = () => {
    ...
    for (
      ...
      items.push(
        <li
          key={number}
          className={`inline-block mx-1 px-3 py-1 bg-primary text-white rounded cursor-pointer ${
            number === currentPage ? "bg-primary-dark" : ""
          }`}
    ...
    if (currentPage - 1 > 1) {
      items.unshift(
        <li
          key="ellipsis-start"
          className="inline-block mx-1 px-3 py-1 bg-gray-200 text-gray-700 rounded"
        >
          <span>...</span>
        </li>,
      );
    ...
    if (currentPage + 1 < totalPages) {
      ...
      items.push(
        <li
          ...
          className={`inline-block mx-1 px-3 py-1 bg-primary text-white rounded cursor-pointer ${
            totalPages === currentPage ? "bg-gray-700" : ""
          }`}
        >
          {totalPages}
        </li>,
  ...
  return (
    <div className="px-32 h-screen w-full flex flex-col">
      <h1 className="text-4xl font-bold my-8 text-primary">
        Solicitudes para ser organizador
      </h1>
          ...
          <table className="table-auto w-2/3 ">
            <thead>
              <tr className="bg-primary">
                <th className="border-2 border-primary px-4 py-2 text-white">
                  Nombre
                </th>
                <th className="border-2 border-primary px-4 py-2 text-white">
                  Email
            ...
            <tbody>
              {currentRows.map((user) => (
                <tr key={user.id}>
                  <td className="border-2 border-primary-light px-4 py-2">
                    {user.name}
                  </td>
                  <td className="border-2 border-primary-light px-4 py-2">
                    {user.email}
